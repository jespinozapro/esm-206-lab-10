...
# must have exact same variable names as original data table
...
# Use this code for final - relevant to a question
```
...
predicted_probs <- predict(donner_blr, newdata = donner_newdata, type = "response",
                           se.fit = TRUE)
...
```{r gganimate_fish}
aq_plot <- ggplot(aq_tidy, aes(x = as.numeric(year), y = aq_mil, group = country)) + geom_line(aes(color = country)) +
